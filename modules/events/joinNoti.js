...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("", event.threadID, () => api.sendMessage({body:`✅ Kết Nối Vào Nhóm ${threadName} Lúc ${session} Thành Công....\n──────────────────\n→ Tổng lệnh hiện có: ${commands.size}\n→ Dấu lệnh: ${global.config.PREFIX}\n→ Phiên bản: ${global.config.version}\n→ Admin: Nguyễn Lê Quốc Huy\n→ Facebook: https://www.facebook.com/qhuy.203\n📌 Sử dụng /help hoặc /menu để xem chi tiết lệnh\n⏰ Thêm bot vào lúc: ${time} 〈 ${thu} 〉`, attachment: fs.createReadStream(__dirname + "/cache/joinGif/join.gif")}, threadID));
  }
  else {
    try {
